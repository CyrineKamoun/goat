...
The generation pipeline:
    ...
    3. For polygon layers: generate a second PMTiles with label anchor points
       using tippecanoe's --convert-polygons-to-label-points, then merge both
       into a single file using tile-join (layers: "default" + "default_anchor")
...
class PMTilesGenerator:
    ...
    def _check_dependencies(self: Self) -> bool:
        ...
        if not shutil.which("tile-join"):
            logger.warning(
                "tile-join not found. Polygon label anchor generation will be disabled. "
                "Install tippecanoe to get tile-join."
            )
        return True
    ...
    def generate_from_table(
        ...
        has_tile_join = shutil.which("tile-join") is not None
        generate_anchors = is_polygon and has_tile_join
        ...
        try:
            with tempfile.NamedTemporaryFile(
                ...
                if generate_anchors:
                    # Polygon layer: generate geometry + anchor tiles separately,
                    # then merge into a single PMTiles with tile-join
                    self._generate_polygon_with_anchors(
                    ...
                    # Non-polygon layer (or tile-join unavailable): single tippecanoe run
    ...
    def _generate_polygon_with_anchors(
        self: Self,
        input_path: str,
        output_path: str,
        snapshot_id: int | None = None,
        ...
        """Generate PMTiles for polygon layers with a label anchor layer.

        Creates two separate PMTiles (geometry + label anchors) then merges
        them into a single file using tile-join. The result contains two MVT
        layers: "default" (polygons) and "default_anchor" (label anchor points).

        Args:
            input_path: Input GeoJSON file path
            output_path: Output PMTiles file path
            snapshot_id: DuckLake snapshot ID to embed in metadata
            show_progress: If True, stream progress to stdout

        Raises:
            subprocess.CalledProcessError: If tippecanoe or tile-join fails
        """
        with (
            tempfile.NamedTemporaryFile(
                suffix=".pmtiles", delete=True, prefix="pmtiles_geom_"
            ) as tmp_geom,
            tempfile.NamedTemporaryFile(
                suffix=".pmtiles", delete=True, prefix="pmtiles_anchor_"
            ) as tmp_anchor,
        ):
            # Step 1: Generate polygon geometry tiles (variable-depth)
            logger.info("Generating polygon geometry tiles...")
            self._run_tippecanoe(
                input_path=input_path,
                output_path=tmp_geom.name,
                geometry_type="POLYGON",
                snapshot_id=snapshot_id,
                show_progress=show_progress,
            )

            # Step 2: Generate label anchor point tiles (variable-depth)
            logger.info("Generating label anchor point tiles...")
            self._run_tippecanoe(
                input_path=input_path,
                output_path=tmp_anchor.name,
                geometry_type="POLYGON",
                snapshot_id=snapshot_id,
                show_progress=show_progress,
                label_anchor=True,
            ...
            # Step 3: Merge both into a single PMTiles using tile-join
            logger.info("Merging geometry and anchor tiles with tile-join...")
            self._run_tile_join(
                output_path=output_path,
                input_paths=[tmp_geom.name, tmp_anchor.name],
                snapshot_id=snapshot_id,
                show_progress=show_progress,
            )
    ...
    def _run_tile_join(
        ...
        """Run tile-join to merge multiple PMTiles into one.

        Args:
            output_path: Output PMTiles file path
            input_paths: List of input PMTiles file paths to merge
            snapshot_id: DuckLake snapshot ID to embed in metadata
            show_progress: If True, stream progress to stdout

        Raises:
            subprocess.CalledProcessError: If tile-join fails
        """
        cmd = [
            "tile-join",
            "-o",
            ...
            "--force",
            "--no-tile-size-limit",
            "--overzoom",  # Scale up tiles from lower-maxzoom tilesets to match higher ones
            "-n",
            output_path,  # Set name to output path (tile-join defaults to concatenated input names)
        ]

        # Embed metadata in description (tile-join overwrites description from inputs)
        description_data: dict[str, str | int] = {
            "generated_at": datetime.now(timezone.utc).isoformat(),
        }
        if snapshot_id is not None:
            description_data["snapshot_id"] = snapshot_id
        cmd.extend(["-N", json.dumps(description_data)])

        cmd.extend(input_paths)

        logger.debug(f"Running tile-join: {' '.join(cmd)}")
        ...
        if show_progress:
            ...
            if result.returncode != 0:
                ...
                logger.error(
                    f"tile-join failed (exit {result.returncode}): {error_msg}\n"
                    f"Command: {' '.join(cmd)}"
                )
                raise subprocess.CalledProcessError(
                    result.returncode, cmd, result.stdout, result.stderr
