...
async def member_thumbnails(
    ...
    The bundle's own thumbnail wins where it has one: the thumbnail task draws
    every member on one map, which is a picture of the bundle rather than of
    one layer inside it, and it is the same map the bundle's preview shows.

    Falling back to a member's own thumbnail, named by the type's spec
    (``thumbnail_role``), for every bundle the task has not drawn yet — one
    imported before this existed, or one whose members are all tabular. That
    keeps a real picture up instead of a placeholder while the schedule catches
    up, and means no backfill has to run for the change to be safe.

    Derived on every read instead of copied onto the bundle: either thumbnail
    is regenerated when its data changes, and a copy would keep showing the
    network as it looked at import.
    """
    if not bundle_ids:
        return {}
    rows = (
