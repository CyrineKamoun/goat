...
class CRUDLayer(CRUDLayerBase):
    ...
    async def delete(
        ...
        # Delete layer thumbnail (generated by goatlib under thumbnails/layers/
        # in the data bucket; stored as a raw S3 key, not a full URL)
        if (
            layer.thumbnail_url
            and layer.thumbnail_url.startswith(settings.THUMBNAIL_DIR_LAYER)
            and settings.TEST_MODE is False
        ):
            settings.S3_CLIENT.delete_object(
                Bucket=settings.S3_BUCKET_NAME,
                Key=layer.thumbnail_url,
            )
