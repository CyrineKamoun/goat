CREATE OR REPLACE FUNCTION accounts.authorization(
    user_id UUID,
...
BEGIN
    ...
    extracted_params := accounts.extract_params_request(requested_resource, requested_path);
    ...
    rec_resource := accounts.check_resource(requested_resource, requested_path, requested_method);

    /*Get user into a record*/
    rec_user := accounts.check_user(user_id, rec_resource);
    
    /*Get organization into a record*/
    rec_organization := accounts.check_organization(rec_user, rec_resource, organization_id);

    /*Check team*/
    IF team_ids IS NOT NULL THEN
        PERFORM accounts.check_team(rec_user.id, team_ids);
    END IF;
    
    /*Check if user has access to the layers*/
    IF layer_ids IS NOT NULL THEN
        PERFORM accounts.check_layer(rec_resource.id, rec_user.id, rec_user.organization_id, layer_ids);
    END IF;

    /*Check if user has access to the projects*/
    IF project_ids IS NOT NULL THEN
        PERFORM accounts.check_project(rec_resource.id, rec_user.id, rec_user.organization_id, project_ids);
    END IF;
